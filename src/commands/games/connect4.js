...
module.exports = new simpleCommand(
    async(message, args, client, addCD) => {
        if (!message.guild.me.permissions.has("MANAGE_MESSAGES")) return message.channel.send("I need the manage messages permission to play this game")
...
function ask(betmoney, user1d, user2d, message) {
    ...
    var x = false
    const filter = m => m.author.id == user2d.id;
    const collector67 = message.channel.createMessageCollector({ filter, max: 1, time: 20000 })
            ...
            api.changeBal(winner.id, betmoney)
                .then(() => {
                    ...
                    .setColor('#5d369d')
                        ...
                        .setFooter(`"This game sucks, I want a rematch" -${loser.name}`)
