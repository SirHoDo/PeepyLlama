...
module.exports = new simpleCommand(
    async(message, args, client, addCD) => {
        ...
        if (isNaN(amount) || amount < 1 || !Number.isInteger(amount)) {
            ...
            message.channel.send("Please use command like this `>gift <itemname> <amount> <@user>`\n**Remember, >gift is for giving ITEMS, while >give is for giving COINS**")
        } else {
            if (user2) {
                    ...
                    api.getUser(user1.id)
                        .then((user) => {
                            if (items.hasOwnProperty(item)) {
                                if (!user.hasOwnProperty("inv")) user.inv = {};
                                if (user.inv.hasOwnProperty(item)) {
                                    ...
                                    if (items[item][5].customgive) {
                                        const embed = new Discord.MessageEmbed()
                                            .setColor('#5d369d')
                                            .setTitle("Now why would you do that?")
                                            .setDescription("You cannot gift this item.")
                                            ...
                                            api.getUser(user2.id)
                                                .then((user2) => {
                                                    ...
                                                    api.modUser(user1.id, user)
                                                        .then(() => {
                                                            if (!user2.hasOwnProperty("inv")) user2.inv = {};
        ...
        cooldown: 0,
        cooldownMessage: "",
        usage: "{prefix}{cmd} <item> <amount> <@user>",
        perms: ["SEND_MESSAGES"],
        description: "Allows you to gift an item to a user!"
    }
)