...
class Board{
  constructor(player1, player2, channel, win, draw, betmoney){
    ...
    this.channel.send(new Discord.MessageEmbed().setColor('#5d369d').setTitle("Setting the board!!")).then(m => {
      this.m = m
      this.playing = true
      this.winf = win
      this.draw = draw
  ...
  if(!b) return false;
    ...
    this.m.awaitReactions((reaction, user)=>['1⃣', '2⃣', '3⃣', '4⃣', '5⃣', '6⃣', '7⃣'].includes(reaction.emoji.name) && user.id === this.id, { max: 1, time: 120000})
        ...
        .catch(() => {
          ...
          this.m.edit(new Discord.MessageEmbed().setColor('#5d369d').setTitle("Times up!"))
          this.mex.delete()
